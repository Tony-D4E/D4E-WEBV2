...
const stats = [
  ...
  { valeur: 'Temps réel', label: 'Suivi des compteurs', icon: BarChart3 },
];
...
const fonctionnalites = [
  {
    ...
    desc: 'Location, maintenance, MPS : définissez vos règles de facturation par contrat. Engagement avec régularisation, prix dégressif, forfait global.',
    couleur: '#F59E0B',
  },
  {
    icon: BarChart3,
    titre: 'Facturation automatique des compteurs',
    desc: 'Les relevés de compteurs remontent automatiquement dans Odoo. Les factures se génèrent selon les conditions du contrat, sans saisie manuelle.',
    couleur: '#00D4C8',
  },
  {
    icon: Wrench,
    titre: 'Suivi des interventions SAV',
    desc: 'Planifiez les dépannages, documentez chaque intervention avec rapport détaillé et pièces utilisées, collectez la signature client sur place.',
    couleur: '#F59E0B',
  },
  {
    icon: Truck,
    titre: 'Planification des techniciens',
    desc: 'Organisez les tournées, suivez la disponibilité des équipes et des pièces détachées en temps réel. Alertes en cas de surcharge.',
    couleur: '#00D4C8',
  },
  {
    icon: TrendingUp,
    titre: 'Rentabilité par contrat',
...
const integrations = [
  {
    nom: 'KPAX Manage',
    desc: "Collecte automatique des compteurs via boîtier Liberty ou agent logiciel. Niveaux d'encre, alertes consommables, commandes automatiques.",
    type: 'Collecte & supervision',
  },
  {
    nom: 'MPS Monitor',
    desc: 'Supervision de flotte à distance. Relevés de compteurs, alertes proactives, rapports de performance pour vos clients.',
    type: 'Gestion de flotte',
  },
  {
    nom: 'Katun KDFM',
    desc: "Solutions d'impression gérée. Intégration des données de consommation et de maintenance dans votre cycle de facturation.",
...
const pourQui = [
  {
    ...
    desc: "Vous vendez, louez et maintenez des systèmes d'impression. ElvyPrint centralise vos contrats, votre SAV et votre facturation dans un seul outil.",
    exemples: ['Copieurs multifonctions', 'Imprimantes grand format', "Solutions d'impression", 'Consommables & pièces'],
    couleur: '#F59E0B',
  },
  {
    icon: Settings,
    titre: 'Prestataires MPS',
    desc: "Vous gérez des flottes d'impression pour vos clients. ElvyPrint automatise le suivi des compteurs, la facturation au volume et le reporting.",
    exemples: ['Managed Print Services', 'Infogérance impression', 'Audit de parc', 'Optimisation des coûts'],
    couleur: '#00D4C8',
  },
];
  ...
  return (
    <div className="min-h-screen bg-white pt-20">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#F59E0B]/20 border border-[#F59E0B]/40 rounded-full mb-8">
                ...
                <span className="text-sm font-semibold text-[#F59E0B]">Suite Elvy · Systèmes d'impression</span>
              </div>
              ...
              <p style={{ color: '#FFFFFF', textShadow: '0 2px 12px rgba(0,0,0,0.9)' }} className="text-xl mb-10 leading-relaxed max-w-xl">
                ElvyPrint est la solution Odoo pour les revendeurs de systèmes
                d'impression : contrats, compteurs, interventions et rentabilité
                dans un seul outil. Déploiement clé en main.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-10">
                {[
                  'Facturation des compteurs automatisée, plus de saisie manuelle',
                  'Contrats personnalisables : location, maintenance, forfait page',
                  'Suivi SAV complet avec signature client sur mobile',
                  'Intégrations KPAX et MPS Monitor pour la collecte de données',
                ].map((item) => (
            ...
            <div className="hidden lg:flex flex-col gap-4">
              {[
                { titre: 'Vos compteurs remontent, vos factures se génèrent', desc: 'Intégration directe avec KPAX et MPS Monitor. Plus de relevés manuels.' },
                { titre: 'Chaque intervention documentée et facturée', desc: 'Rapport terrain, pièces utilisées, signature client. La facture suit automatiquement.' },
                { titre: 'Rentabilité visible contrat par contrat', desc: "Marges, surconsommation d'encre, contrats sous-performants : tout est mesuré." },
              ].map((item, i) => (
        ...
        <div ref={featRef} className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-on-scroll ${featVisible ? 'is-visible' : ''}`}>
          <div className="text-center max-w-3xl mx-auto mb-16">
            <div className="inline-flex items-center gap-2 px-5 py-2 bg-[#F59E0B]/10 border border-[#F59E0B]/30 rounded-full mb-4">
              ...
              <span className="text-base font-bold text-[#F59E0B]">Fonctionnalités ElvyPrint</span>
            </div>
            <h2 className="text-4xl font-black text-[#0F1D3A] mb-4">Du contrat à la facture, tout est connecté</h2>
            <p className="text-lg text-gray-500">
              ElvyPrint couvre l'ensemble du cycle de vie de vos contrats d'impression :
              de la signature au renouvellement, en passant par chaque relevé et chaque intervention.
            </p>
          </div>
          ...
          <div className="space-y-0 rounded-3xl overflow-hidden border border-gray-200">
            {fonctionnalites.map((f, i) => (
              <div key={i} className={`p-8 flex gap-6 items-start transition-all ${i % 2 === 0 ? 'bg-[#0F1D3A]' : 'bg-white border-t border-b border-gray-100'}`}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: i % 2 === 0 ? f.couleur + '30' : f.couleur + '15' }}>
                  <f.icon className="h-7 w-7" style={{ color: f.couleur }} />
                </div>
                <div>
                  <h3 className={`text-xl font-black mb-2 ${i % 2 === 0 ? '' : 'text-[#0F1D3A]'}`} style={i % 2 === 0 ? { color: '#FFFFFF' } : {}}>
                    {f.titre}
                  </h3>
                  <p style={i % 2 === 0 ? { color: '#CBD5E1' } : {}} className={`leading-relaxed ${i % 2 === 0 ? '' : 'text-gray-500'}`}>
                    {f.desc}
                  </p>
                </div>
              </div>
      ...
      <section className="py-24 bg-[#0F1D3A] relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute w-[600px] h-[600px] rounded-full -bottom-40 -left-40 opacity-10" style={{ background: 'radial-gradient(circle, #F59E0B, transparent)' }} />
        </div>
        <div ref={intRef} className={`relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-on-scroll ${intVisible ? 'is-visible' : ''}`}>
          <div className="text-center max-w-3xl mx-auto mb-16">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#F59E0B]/20 border border-[#F59E0B]/40 rounded-full mb-4">
              ...
              <span className="text-sm font-semibold text-[#F59E0B]">Connecté à vos outils</span>
            </div>
            <h2 className="text-4xl font-black text-white mb-4">Vos compteurs remontent, Odoo facture</h2>
            <p className="text-gray-300 text-lg">
              ElvyPrint s'interface avec les principales solutions de collecte de compteurs
              et de supervision de flotte du marché.
            </p>
          </div>
            ...
            {integrations.map((integ, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-8 hover:bg-white/10 transition-all">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-xs font-bold px-2 py-1 rounded-full bg-[#F59E0B]/20 text-[#F59E0B]">{integ.type}</span>
                </div>
          ...
          <div className="bg-white/5 border border-[#F59E0B]/30 rounded-3xl p-8 text-center">
            ...
            <p className="text-gray-400 mb-6 max-w-2xl mx-auto">
              ElvyPrint repose sur Odoo : tout système disposant d'une API ou d'un export
              structuré peut être connecté. Parlons-en.
            </p>
      ...
      <section className="py-24 bg-white">
        <div ref={pourQuiRef} className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-on-scroll ${pourQuiVisible ? 'is-visible' : ''}`}>
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-4xl font-black text-[#0F1D3A] mb-4">Conçu pour les professionnels de l'impression</h2>
            <p className="text-lg text-gray-500">
              Que vous soyez revendeur, distributeur ou prestataire MPS,
              ElvyPrint s'adapte à votre modèle économique.
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            {pourQui.map((p, i) => (
              <div key={i} className="rounded-3xl p-8 border-2 hover:shadow-lg transition-all" style={{ borderColor: p.couleur + '40', backgroundColor: p.couleur + '08' }}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6" style={{ backgroundColor: p.couleur + '20' }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {p.exemples.map((ex, j) => (
                    <span key={j} className="px-3 py-1 rounded-full text-sm font-semibold" style={{ backgroundColor: p.couleur + '20', color: p.couleur }}>
                      {ex}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {equipe.map((p, i) => (
              <div key={i} className="bg-white rounded-3xl p-8 flex flex-col items-center text-center border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-500">
                <img src={p.photo} alt={p.nom} className="w-32 h-32 rounded-full object-cover mb-6 shadow-lg" style={{ border: `4px solid ${p.couleur}` }} />
                ...
                <div className="flex flex-col gap-3 w-full">
                  <a href={`tel:${p.tel}`} className="flex items-center justify-center gap-2 px-5 py-3 text-white font-bold rounded-xl transition-all" style={{ backgroundColor: p.couleur }}>
                    <Phone className="h-4 w-4" />{p.telDisplay}
                  </a>
                  <a href={`https://wa.me/${p.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-5 py-3 bg-[#25D366] hover:bg-[#1fb855] text-white font-bold rounded-xl transition-all">
                  ...
                  <a href={`mailto:${p.email}`} className="flex items-center justify-center gap-2 px-5 py-3 bg-white hover:bg-gray-50 text-gray-700 font-semibold rounded-xl transition-all border border-gray-200">
      ...
      <section id="contact" className="py-24 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <form action="https://formspree.io/f/xdawrgoz" method="POST" className="bg-[#0F1D3A] rounded-3xl p-8 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <input type="text" name="prenom" placeholder="Prénom *" required className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors" />
              <input type="text" name="nom" placeholder="Nom *" required className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors" />
            </div>
            <input type="email" name="email" placeholder="Email *" required className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors" />
            <input type="text" name="entreprise" placeholder="Entreprise" className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors" />
            <input type="tel" name="telephone" placeholder="Téléphone" className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors" />
            ...
            <select name="besoin" className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-gray-400 w-full focus:outline-none focus:border-[#F59E0B] transition-colors">
              ...
              <option>Intégration KPAX / MPS Monitor</option>
              <option>Je veux en savoir plus</option>
            </select>
            <textarea name="message" placeholder="Décrivez votre activité et vos besoins *" rows={4} required className="bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 w-full focus:outline-none focus:border-[#F59E0B] transition-colors resize-none" />
